...
class NotificationRepository {
  ...
  static const int _dbVersion =
      3; // Set to 3 and use onOpen to ensure table exists
  ...
  Future<Database> _initDatabase() async {
    try {
      ...
      if (kDebugMode) debugPrint('📦 Initializing Notifications DB at: $path');
      ...
      return await openDatabase(
        ...
        onOpen: (db) async {
          try {
            if (kDebugMode) debugPrint('🔓 Opening DB, verifying schema...');
            ...
            if (tables.isEmpty) {
              if (kDebugMode) {
                debugPrint(
                  '  - Notifications table missing on open, creating...',
                );
              }
              await _createNotificationsTable(db);
            } else {
              if (kDebugMode) debugPrint('  - Notifications table present');
            }
          } catch (e) {
            if (kDebugMode) debugPrint('❌ Error in onOpen DB check: $e');
          }
        },
      );
    } catch (e) {
  ...
  Future<void> _onCreate(Database db, int version) async {
    try {
      await _createNotificationsTable(db);
      if (kDebugMode) {
        debugPrint('✓ Notifications table created (version $version)');
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    try {
      ...
      if (oldVersion < 2) {
        ...
        if (tables.isEmpty) {
          if (kDebugMode) {
            debugPrint('  - Creating missing notifications table...');
          }
          await _createNotificationsTable(db);
        } else {
          if (kDebugMode) debugPrint('  - Notifications table already exists');
        }
      }

      if (kDebugMode) debugPrint('✓ Database upgrade completed');
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> _ensureTableExists(Database db) async {
    try {
      ...
      if (tables.isEmpty) {
        if (kDebugMode) {
          debugPrint('⚠️  _ensureTableExists: table missing, creating...');
        }
        await _createNotificationsTable(db);
      } else {
        if (kDebugMode) debugPrint('✔️  _ensureTableExists: table present');
      }
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Error in _ensureTableExists: $e');
  ...
  Future<int> logNotification(NotificationLog notification) async {
    try {
      ...
      final id = await db.insert(_tableName, notification.toJson());

      if (kDebugMode) {
        debugPrint(
          '✓ Logged notification: ${notification.releaseTitle} for ${notification.favoriteTitle}',
        );
      }
      return id;
    } catch (e) {
  ...
  Future<bool> hasBeenNotified(
    ...
    try {
      ...
      final yesterday = DateTime.now().subtract(
        const Duration(hours: oneDayAgo),
      );

      // Baue die WHERE-Klausel dynamisch auf, um NULL-Werte richtig zu behandeln
      String where =
          'favoriteTitle = ? AND releaseTitle = ? AND notifyTime > ?';
      ...
      if (kDebugMode) debugPrint('❌ Error checking notification history: $e');
      return false;
    }
  }
    ...
    try {
      ...
      if (hashResult.isNotEmpty) {
        if (kDebugMode) {
          ...
          debugPrint(
            '⏭️  [DEDUP] Exact contentHash found (sent on $existingTime) - SKIP: $contentHash',
          );
        }
        return true;
      }
      ...
      if (kDebugMode) {
        debugPrint(
          '✅ [DEDUP] New content (no matching contentHash) - WILL SEND: $favoriteTitle / $releaseTitle / $episodeNumber',
        );
      }
      return false;
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Error checking duplicate: $e');
  ...
  Future<Map<String, dynamic>> getNotificationStats({int hoursBack = 2}) async {
    try {
      ...
      if (kDebugMode) debugPrint('❌ Error getting notification stats: $e');
      return {};
    }
  }

  /// Holt die Benachrichtigungs-Historie (optional gefiltert nach Favorit)
  Future<List<NotificationLog>> getHistory({
    String? favoriteTitle,
    int limit = 100,
  }) async {
    try {
      ...
      if (kDebugMode) debugPrint('❌ Error getting history: $e');
      return [];
    }
  }
  ...
  Future<bool> markAsShown(int notificationId) async {
    try {
      ...
      if (kDebugMode) debugPrint('❌ Error marking as shown: $e');
      return false;
    }
  }
  ...
  Future<int> getUnshownCount() async {
    try {
      ...
      if (kDebugMode) debugPrint('❌ Error getting unshown count: $e');
      return 0;
    }
  }

  /// Löscht alte Benachrichtigungen (älter als X Tage)
  Future<int> _cleanupOldNotifications({
    int retentionDays = _maxRetentionDays,
  }) async {
    try {
      final db = await database;
      ...
      if (deleted > 0 && kDebugMode) {
        debugPrint(
          '🗑️  Cleaned up $deleted old notifications (older than $retentionDays days)',
        );
      }

      return deleted;
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Error cleaning up old notifications: $e');
      return 0;
    }
  }
  ...
  Future<int> deleteByFavoriteTitle(String favoriteTitle) async {
    try {
      ...
      if (kDebugMode) {
        debugPrint('✓ Deleted $deleted notifications for: $favoriteTitle');
      }
      return deleted;
    } catch (e) {
      if (kDebugMode) debugPrint('❌ Error deleting by favorite: $e');
      return 0;
    }
  }
  ...
  Future<int> deleteAllNotifications() async {
    try {
      ...
      if (kDebugMode) {
        debugPrint('✓ Deleted all notifications (count: $deleted)');
      }
      return deleted;
    } catch (e) {
  ...
  Future<void> close() async {
    if (_database != null) {
      ...
      _database = null;
      if (kDebugMode) {
        debugPrint('✓ Notifications database closed');
      }
    }
  }
}
